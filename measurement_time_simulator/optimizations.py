...
if HAS_NUMBA:
    ...
    def optimized_gamma_base(T: float, T_c_array, N_array, N_Q: float, 
                            GAMMA_BASE_COEFF: float, N_LOG_DIVISOR: float, 
    ...
    @jit(nopython=True, cache=True, fastmath=True)
    def optimized_statistics_factor(stats_type_array, T_F_array, T_c_bose_array, 
                                   temperature: float, FERMION_COEFF: float, 
                                   BOSON_COEFF: float):
        """
    ...
    @jit(nopython=True, cache=True, fastmath=True)
    def optimized_correlation_factor(U_array, W_array, temperature: float, 
                                    T_star_array, CORRELATION_COEFF: float):
        """
    ...
    def optimized_gamma_base(T, T_c_array, N_array, N_Q, GAMMA_BASE_COEFF, 
                            N_LOG_DIVISOR, MIN_TEMPERATURE):
    ...
    def optimized_statistics_factor(stats_type_array, T_F_array, T_c_bose_array,
                                   temperature, FERMION_COEFF, BOSON_COEFF):
        """Fallback version without JIT"""
    ...
    def optimized_correlation_factor(U_array, W_array, temperature, T_star_array, 
                                    CORRELATION_COEFF):
        """Fallback version without JIT"""
        return 1 + CORRELATION_COEFF * (U_array / W_array) * np.exp(-temperature / T_star_array)
...
class OptimizedCalculator:
    ...
    def gamma_base_cached(self, T, T_c, N, N_Q, GAMMA_BASE_COEFF, 
                         N_LOG_DIVISOR, MIN_TEMPERATURE):
        """
...
def batch_compute_with_chunking(compute_func, arrays, chunk_size=10000):
    ...
    for i in range(n_chunks):
        ...
        chunk_arrays = {
            key: arr[start_idx:end_idx] for key, arr in arrays.items()
        }
