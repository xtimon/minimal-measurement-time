...
class GPUInformationMeasurementSimulator:
    ...
    def __init__(self, temperature: float = 300.0, use_gpu: bool = True, suppress_logging: bool = False):
        """
    ...
    def _calculate_detector_response_time(
        self, stats_type_array: np.ndarray, N_array: np.ndarray, custom_detector_time: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """
        ...
        detector_times = xp.where(mask_fermion, DETECTOR_TIME_FERMION * log_factor, detector_times)

        # Бозоны: 100 нс базовое время (медленнее из-за конденсата)
        mask_boson = stats_type_array == 1
        detector_times = xp.where(mask_boson, DETECTOR_TIME_BOSON * log_factor, detector_times)

        # Классические: 10 нс базовое время
        mask_classical = stats_type_array == 2
        detector_times = xp.where(mask_classical, DETECTOR_TIME_CLASSICAL * log_factor, detector_times)

        return xp.maximum(detector_times, MIN_DETECTOR_RESPONSE_TIME)

    def _calculate_equilibrium_time(
        self,
        stats_type_array: np.ndarray,
        T_c_array: np.ndarray,
        N_array: np.ndarray,
        U_array: Optional[np.ndarray] = None,
        custom_equilibrium_time: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """
        ...
        equilibrium_times = xp.where(mask_fermion, base_time_fermion * temp_factor * N_factor, equilibrium_times)

        # Бозоны: 1 мкс базовое время (медленнее из-за конденсации)
        mask_boson = stats_type_array == 1
        base_time_boson = EQUILIBRIUM_TIME_BOSON
        equilibrium_times = xp.where(mask_boson, base_time_boson * temp_factor * N_factor, equilibrium_times)

        # Классические: 1 нс базовое время
        mask_classical = stats_type_array == 2
        base_time_classical = EQUILIBRIUM_TIME_CLASSICAL
        equilibrium_times = xp.where(mask_classical, base_time_classical * temp_factor * N_factor, equilibrium_times)
        ...
        if U_array is not None:
            ...
            equilibrium_times = xp.where(
                mask_correlated, base_time_correlated * U_factor * temp_factor, equilibrium_times
            )

        return xp.maximum(equilibrium_times, MIN_EQUILIBRIUM_TIME)

    def _calibrate_noise_parameters(
        self, stats_type_array: np.ndarray, N_array: np.ndarray, T_c_array: np.ndarray, delta_I_array: np.ndarray
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def _calculate_decoherence_factor(
        self, T1_array: np.ndarray, T2_array: np.ndarray, measurement_time: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _calculate_noise_factor(
        self,
        temperature: float,
        noise_temperature: np.ndarray,
        shot_noise_factor: np.ndarray,
        technical_noise: np.ndarray,
        ...
        measurement_time: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """
    ...
    def gamma_total_batch(
        ...
        stats_f = xp.where(
            stats_type_array == 0,  # FERMION
            1 + FERMION_COEFF * (T_F_array / self.temperature),
            xp.where(stats_type_array == 1, 1 + BOSON_COEFF * (T_c_bose_array / self.temperature), 1.0),  # BOSON
        )  # CLASSICAL
    ...
    def _gamma_base_vectorized(
        self, T: float, T_c_array: np.ndarray, N_array: np.ndarray, N_Q: float = DEFAULT_N_Q
    ) -> np.ndarray:
    ...
    def main_equation_batch(
        ...
        if include_decoherence:
            ...
            for iteration in range(3):  # Несколько итераций для сходимости
                decoherence_factor = self._calculate_decoherence_factor(T1_array, T2_array, min_tau_current)
        ...
        if include_noise:
            ...
            if (
                flicker_noise_factor_array is None
                or quantum_noise_factor_array is None
                or environment_noise_factor_array is None
            ):
                calibrated_noise = self._calibrate_noise_parameters(stats_type_array, N_array, T_c_array, delta_I_array)
        ...
        if equilibrium_time_array is None:
            equilibrium_time_array = self._calculate_equilibrium_time(stats_type_array, T_c_array, N_array, U_array)
        else:
        ...
        if detector_response_time_array is None:
            detector_response_time_array = self._calculate_detector_response_time(stats_type_array, N_array)
        else:
    ...
    def simulate_single_system(
        ...
        min_tau = self.main_equation_batch(
            ...
            flicker_noise_factor_array=(
                np.array([flicker_noise_factor]) if (include_noise and flicker_noise_factor is not None) else None
            ),
            quantum_noise_factor_array=(
                np.array([quantum_noise_factor]) if (include_noise and quantum_noise_factor is not None) else None
            ),
            environment_noise_factor_array=(
                np.array([environment_noise_factor])
                if (include_noise and environment_noise_factor is not None)
                else None
            ),
            equilibrium_time_array=np.array([equilibrium_time]) if equilibrium_time is not None else None,
            detector_response_time_array=(
                np.array([detector_response_time]) if detector_response_time is not None else None
            ),
        ...
        if include_noise:
            ...
            if flicker_noise_factor is None or quantum_noise_factor is None or environment_noise_factor is None:
                calibrated = self._calibrate_noise_parameters(
                    np.array([stats_type]), np.array([N]), np.array([T_c]), np.array([delta_I])
                )
        ...
        results = {
            ...
            "parameters": {
                ...
                "flicker_noise_factor": (
                    flicker_noise_factor if (include_noise and flicker_noise_factor is not None) else None
                ),
                "quantum_noise_factor": (
                    quantum_noise_factor if (include_noise and quantum_noise_factor is not None) else None
                ),
                "environment_noise_factor": (
                    environment_noise_factor if (include_noise and environment_noise_factor is not None) else None
                ),
